...
import { searchExpenditures } from '@/lib/database';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const payee_state = searchParams.get('payee_state');
    ...
    const filters = {
      ...
      payee_state: payee_state || undefined,
    ...
    const result = await searchExpenditures(query, filters, page, limit);
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to search expenditures' },
      { status: 500 }
    );
  }
} 