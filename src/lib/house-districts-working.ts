...
async function getCachedHumanityScore(personId: string): Promise<number> {
  try {
    ...
    // Calculate new score
    console.log(`Calculating humanity score for ${personId}...`);
    ...
    console.log(`Cached humanity score for ${personId}: ${score}`);
...
export async function getHouseDistrictsData(): Promise<{ success: boolean; data?: HouseDistrict[]; error?: string }> {
  try {
    console.log('=== STARTING HOUSE DISTRICTS WORKING VERSION ===');
    ...
    for (const district of districtsResult.data) {
      try {
        ...
        console.log(`District status: ${district.status}, representative: ${district.representative}`);
