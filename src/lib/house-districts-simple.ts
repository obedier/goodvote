...
export async function getHouseDistrictsData(): Promise<{ success: boolean; data?: HouseDistrict[]; error?: string }> {
  try {
    console.log('Starting house districts data fetch...');
    
    // First, test if congressional data is accessible
    const testQuery = `
    ...
    // Get all House districts from the authoritative congressional data
    const districtsQuery = `
      ...
      FROM cng_district_mapping 
      WHERE district > 0  -- Exclude Senate seats
    ...
    for (const district of districtsResult.data) {
      try {
        ...
        // For filled seats, try to find FEC data
        ...
        if (district.status === 'FILLED' && district.representative !== 'VACANT') {
          ...
          if (fecMatchResult.success && fecMatchResult.data.length > 0) {
            ...
            console.log(`No FEC match found for: ${district.representative}`);
          }
        }
